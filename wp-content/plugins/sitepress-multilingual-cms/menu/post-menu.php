...
$post_type_label = $wp_post_types[ $post->post_type ]->labels->singular_name != "" ? $wp_post_types[ $post->post_type ]->labels->singular_name : $wp_post_types[ $post->post_type ]->labels->name;
...
<?php
$disabled_original_selection = ( ( empty( $_GET[ 'action' ] ) || $_GET[ 'action' ] != 'edit' ) && $trid ) ? ' disabled="disabled"' : false;
...
if (isset($translations) && count($translations) == 1 && count(SitePress::get_orphan_translations($trid, $post->post_type, $this->get_current_language())) > 0){
	...
	?>
	<?php if ( ! $disabled_original_selection ) : ?>
		<div id="icl_document_connect_translations_dropdown" class="icl_box_paragraph">
			...
			<input type="hidden" id="icl_connect_translations_trid" name="icl_connect_translations_trid"
			       value="<?php echo $trid; ?>"/>
			<input type="hidden" id="icl_connect_translations_post_type" name="icl_connect_translations_post_type"
			       value="<?php echo $post->post_type; ?>"/>
			<input type="hidden" id="icl_connect_translations_language" name="icl_connect_translations_language"
			       value="<?php echo $this->get_current_language(); ?>"/>
	...
	<div id="translation_of_wrap">
		...
		if (!$is_original && ( $selected_language != $source_language || ( isset( $_GET[ 'lang' ] ) && $_GET[ 'lang' ] != $source_language ) ) && 'all' != $this->get_current_language() ) {
			...
			<div id="icl_translation_of_panel" class="icl_box_paragraph">
				...
				<select name="icl_translation_of" id="icl_translation_of"<?php echo $disabled_original_selection; ?>>
					<?php
				...
				<?php
				if ( $disabled_original_selection ) {
					?>
