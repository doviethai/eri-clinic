...
class SitePress
{
	...
	function init()
	{
		...
		if (!empty( $this->settings[ 'existing_content_language_verified' ] ) && (!is_admin() || SitePress::check_settings_integrity()) ) {
			...
			$using_root_page = $this->settings[ 'language_negotiation_type' ] == 1
			...
			if ( defined( 'WP_ADMIN' ) ) {
				...
				} elseif ( isset( $current_user->data ) && isset( $current_user->data->ID ) ) {
	...
	function save_post_actions( $pidd, $post )
	{
	  ...
	  if ($original_id == $post_id) {
		...
		if ( ! empty( $this->settings[ 'sync_post_date' ] ) ) {
			if ( ! $source_language || $source_language == $language_code ) {
				$post_date   = $wpdb->get_var( $wpdb->prepare( "SELECT post_date FROM {$wpdb->posts} WHERE ID=%d",
				                                               $post_id ) );
				$ids_to_sync = $wpdb->get_col( $wpdb->prepare( "SELECT element_id
													FROM {$wpdb->prefix}icl_translations
													WHERE source_language_code IS NOT NULL
				...
				$post_date   = $wpdb->get_var(
					$wpdb->prepare( "	SELECT post_date
										...
										LIMIT 1) ",
					                $post_id ) );
				$ids_to_sync = array( $post_id );
			}
			foreach ( $ids_to_sync as $tp ) {
...
//		}
	...
	{            
		...
		if ( $this->settings[ 'sync_delete' ] ) {
			...
			if ( $lang_details->source_language_code || ! $lang_details->trid ) {
				return;
			}
			$translations = $this->get_element_translations( $lang_details->trid, 'post_' . $post_type );
			foreach ( $translations as $t ) {
	...
	function language_filter()
	{
		...
		if ($languages !== false) {
			foreach ( (array) $languages as $code => $count ) {
				if ( $code !== 'all' && ! isset( $active_languages[ $code ] ) ) {
					unset( $languages[ $code ] );
					$languages[ 'all' ] -= $count;
				}
			}
