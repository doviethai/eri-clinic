...
class TranslationManagement{
	...
	function init(){
		...
		if ( ! $user || empty( $user->data ) || ( empty( $user->data->display_name )
...
//            }
	...
	*/
	...
	function _get_element_type_prefix($post_id) {
		if (is_string($post_id) && strcmp(substr($post_id, 0, strlen('external_')), 'external_')===0) {
			return 'package_';
		} else {
			return 'post_';
	...
	function send_jobs($data){
		...
		foreach($selected_posts as $post_id){
			...
			$element_type_prefix = $this->_get_element_type_prefix($post_id);
			$post_trid = $sitepress->get_element_trid($post->ID, $element_type_prefix . $post->post_type);
			$post_translations = $sitepress->get_element_translations($post_trid, $element_type_prefix . $post->post_type);
			...
			foreach($selected_languages as $lang => $action){
				...
				if($action == 2){
					...
					$current_translation_status = $this->get_element_translation($post_id, $lang, $element_type_prefix . $post->post_type);
					...
					if(empty($post_translations[$lang])){
						$translation_id = $sitepress->set_element_language_details(null , $element_type_prefix . $post->post_type, $post_trid, $lang, $translate_from);
					}else{
