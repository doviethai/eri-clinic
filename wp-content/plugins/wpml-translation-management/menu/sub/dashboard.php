...
foreach ( $icl_post_types as $type => $type_info ) {
	$type_in_filter              = isset( $icl_translation_filter[ 'type' ] ) && $type == $icl_translation_filter[ 'type' ];
	$no_selected_type_in_filter  = ! isset( $icl_translation_filter[ 'type' ] ) || ! $icl_translation_filter[ 'type' ];
	$is_external_type            = isset( $type_info->external_type ) && $type_info->external_type;
	$type_in_filter_or_no_filter = $type_in_filter || $no_selected_type_in_filter;

	if ( $type_in_filter_or_no_filter && $is_external_type ) {
		$icl_documents = apply_filters( 'WPML_get_translatable_items', $icl_documents, $type, $icl_translation_filter );
	}
	if ( $is_external_type ) {
	            ...
	            foreach($icl_documents as $doc) {
		            ...
		            <tr<?php if ( $oddcolumn ): ?> class="alternate"<?php endif;?>>
			            ...
			            <td scope="row" class="post-title column-title">
				            ...
				            $wc = apply_filters( 'WPML_estimate_word_count', $wc, $doc, $icl_translation_filter[ 'from_lang' ] );
				            ?>
				            <span id="icl-cw-<?php echo $doc->post_id ?>" style="display:none"><?php echo $wc;
					            $wctotal += $wc; ?></span>
			            ...
			            <td scope="row" class="post-date column-date">
				            <?php if ( $doc->post_date ) {
					            echo date( 'Y-m-d', strtotime( $doc->post_date ) );
				            } ?>
			            </td>
			            ...
			            <td scope="row"><?php
				            if ( isset( $icl_post_statuses[ $doc->post_status ] ) ) {
					            echo $icl_post_statuses[ $doc->post_status ];
				            } else {
					            echo $doc->post_status;
				            }
				            ?>
			            </td>
			            <?php
			            if ( $icl_translation_filter[ 'to_lang' ] ) {
				            ?>
				            ...
				            <td scope="row" class="manage-column column-cb check-column" style="padding-top:4px;">
					            <img src="<?php echo ICL_PLUGIN_URL ?>/res/img/<?php echo $_st = $iclTranslationManagement->status2img_filename( $docst ) ?>" width="16" height="16" alt="<?php echo $_st ?>"/>
				            </td>
			            <?php
			            } else {
				            foreach ( $active_languages as $lang ){
					            ...
					            if ( ! isset( $doc->$_prop_up ) ) {
						            $doc->$_prop_up = false;
					            }
					            if ( ! isset( $doc->$_prop_st ) ) {
						            $doc->$_prop_st = ICL_TM_NOT_TRANSLATED;
					            }

					            switch ( intval( $doc->$_prop_st ) ) {
						            case ICL_TM_NOT_TRANSLATED :
							            $tst_title = esc_attr( __( 'Not translated', 'wpml-translation-management' ) );
							            break;
						            case ICL_TM_WAITING_FOR_TRANSLATOR :
							            $tst_title = esc_attr( __( 'Waiting for translator', 'wpml-translation-management' ) );
							            break;
						            case ICL_TM_IN_PROGRESS :
							            $tst_title = esc_attr( __( 'In progress', 'wpml-translation-management' ) );
							            break;
						            case ICL_TM_DUPLICATE :
							            $tst_title = esc_attr( __( 'Duplicate', 'wpml-translation-management' ) );
							            break;
						            case ICL_TM_COMPLETE :
							            $tst_title = esc_attr( __( 'Complete', 'wpml-translation-management' ) );
							            break;
						            default:
							            $tst_title = '';
					            }
            ...
            <tr>
                <td>
	                ...
	                <?php
		                ...
		                $message = __('You currently have no active alternate languages!','wpml-translation-management');
